...
def inputObs(obstacle_list):
    ...
    f = open("input/obstacle_11.txt", "r")
    obstacle = []
    for line in f:
        if line == '-1' or line == "-1\n":
...
def plotline(line):
    tmp1 = line.boundary[0]
    tmp2 = line.boundary[1]
    x_values = [tmp1.x, tmp2.x]
    y_values = [tmp1.y, tmp2.y]
...
if __name__ == "__main__":
    ...
    fig = voronoi_plot_2d(vor, show_vertices=False,
                          point_size=1.5, line_width=1.5, line_alpha=0.6, show_points=False)
